...
namespace Common
{
    public class MySqlHelper
    {
        ...
        public static string MYSQL_SERVER
        {
            get
            {
                if (String.IsNullOrEmpty(_MYSQL_SERVER))
                {
                    lock (_MYSQL_SERVER)
                    {
                        _MYSQL_SERVER = ReadNodeValue("crm");
                    }
                }
        ...
        ///// 数据文件配置路径
        ///// </summary>
        private static string _XmlPath = AppDomain.CurrentDomain.BaseDirectory + "/Data/data";


        //  private static string _XmlPath = MapPath("~/Data/data");

        public static string XmlPath
        {
            get { return MySqlHelper._XmlPath; }
        }
        ...
        /// <returns></returns>
        public static string ReadNodeValue(string xPath)
        {
            XmlDocument docXml = new XmlDocument();
            docXml.Load(_XmlPath);

            XmlNode node = docXml.DocumentElement.SelectSingleNode(xPath);
            if (node != null)
            {
