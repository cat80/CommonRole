...
namespace Common
{
    ...
    public class Utils
    {
        ...
        {
            ResponseFile(filepath, filename, filetype, true);
        }
        ...
        /// <param name="filename">输出的文件名</param>
        /// <param name="filetype">将文件输出时设置的ContentType</param>
        /// <param name="autoEndResponse">true 自动终止输出 false不会自动终止输出</param>
        public static void ResponseFile(string filepath, string filename, string filetype, bool autoEndResponse)
        {
            Stream iStream = null;
            ...
            try
            {
                ...
                HttpContext.Current.Response.AddHeader("Content-Disposition", "attachment;filename=" + HttpContext.Current.Server.UrlEncode(filename.Trim()).Replace("+", "%20"));
                // else
        ...
        public static string FileMD5(string filePath)
        {
            ...
            {
                throw new ArgumentException("filePath不存在");
            }
            //FileMode mode, FileAccess access, FileShare share
            using (FileStream stream = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                byte[] b = new MD5CryptoServiceProvider().ComputeHash(stream);
                return BitConverter.ToString(b).ToUpper().Replace("-", "");
            }
        ...
        /// 转义xml元素
        /// </summary>
        /// <param name="init">初始内容</param>
        /// <returns></returns>
        public static string EncodeXml(string init)
        {
            ...
            replaceKeys.Add("'", "&apos;");
            replaceKeys.Add("\"", "&quot;");
            foreach (var item in replaceKeys)
            {
                init = init.Replace(item.Key, item.Value);
            }
            return init;
